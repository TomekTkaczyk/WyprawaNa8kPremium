...
namespace WyprawaNa8kPremiumXUnitTests
{
    public class TreeTests
    {
        ...
        public void LovestLevelLeafsSummary_should_by_return_expected_value(int expected, params int?[] arr)
        {
            var tree = new TreeWithLeafs(arr);
            var result = tree.LovestLevelLeafsSummary();

            Assert.Equal(expected, result);
        }

        [Fact]
        public void When_value_of_node_is_large_than_100_should_by_throw_exception()
        {

            int?[] arr = { 101 };
            var tree = new TreeWithLeafs(arr);

            Assert.Throws<ArgumentOutOfRangeException>(() => tree.LovestLevelLeafsSummary());
        }

        [Fact]
        public void When_size_of_tree_is_large_than_10000_should_by_throw_exception()
        {
